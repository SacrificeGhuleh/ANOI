...
void Classifier::preprocess() {
  ...
  for (int row = 0; row < featuresImg.rows; row++) {
    for (int col = 0; col < featuresImg.cols; col++) {
      featuresImg.at<cv::Vec3b>(row, col) = cv::Vec3b(255, 255, 255);
    }
  }
}
...
void Classifier::show() {
  ...
  cv::imshow("FeaturesImage", featuresImg);
...
void Classifier::prepareVisualization() {
  for (Object *obj : objects) {
    for (const cv::Point &point : obj->getIndexes()) {
      ...
      featuresImg.at<cv::Vec3b>(obj->getFeatureOne() * featuresImg.cols,
                                obj->getFeatureOne() * featuresImg.cols) = obj->getColor();
    }
  }
}
...
Classifier::Classifier(std::string &inputImagePath) {
  ...
  featuresImg = cv::Mat_<cv::Vec3b>(FEATURES_MAT_SIZE, FEATURES_MAT_SIZE);
