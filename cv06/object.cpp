...
Object::Object(unsigned int index, cv::Mat_<uint8_t> *filterImg) : index_(index),
                                                                   ...
                                                                   featureOne_(0),
                                                                   featuresVector_(cv::Point(0, 0)),
                                                                   objectClass_(nullptr) {
...
void Object::addPoint(const cv::Point &point) {
  indexes_.emplace_back(point);
}

const cv::Vec3b &Object::getColor() const {
  return color_;
}
...
const std::vector<cv::Point> &Object::getIndexes() const {
  return indexes_;
}

const std::vector<cv::Point> &Object::getPerimeterPoints() const {
  return perimeterPoints_;
}

cv::Mat_<uint8_t> *Object::getFilterImg() const {
  return filterImg_;
}

const cv::Point2d Object::getFeaturesVector() const {
  return featuresVector_;
}

const cv::Point2d Object::computeFeaturesVector() {
  featuresVector_ = cv::Point2d(computeFeatureOne(), computeFeatureTwo());
